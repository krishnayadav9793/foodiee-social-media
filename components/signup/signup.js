...
const FoodieSignupCard = () => {
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden  flex items-center justify-center font-sans">
      ...
      {/* --- Background: Ambient 'Aurora' Gradients (Warm Theme) --- */}
      ...
      <div className="perspective-1000 z-10 w-full max-w-md px-4" 
        ...
        >
          ...
          {/* Floating Decor Icons (CSS Animated - Signup Icons) */}
          ...
          <form onSubmit={handleSubmit} className="space-y-5 relative z-20">
            ...
            <div className="relative group">
                <input 
                  type="text" 
                  required
                  className="peer w-full bg-black/20 border-2 border-transparent focus:border-[#CBF928] rounded-2xl px-5 py-4 text-white placeholder-transparent outline-none transition-all font-medium"
                  placeholder="Full Name"
                />
                <label 
                    className="absolute left-5 top-4 text-white/50 text-sm font-bold transition-all pointer-events-none peer-focus:-translate-y-7 peer-focus:scale-75 peer-focus:text-[#CBF928] origin-left peer-placeholder-shown:translate-y-0 peer-placeholder-shown:scale-100 peer-placeholder-shown:text-white/50"
                >
                  Full Name
                </label>
                {/* Icon */}
            ...
            <div className="relative group">
              ...
              />
              <label 
                className="absolute left-5 top-4 text-white/50 text-sm font-bold transition-all pointer-events-none peer-focus:-translate-y-7 peer-focus:scale-75 peer-focus:text-[#FF9F1C] origin-left peer-placeholder-shown:translate-y-0 peer-placeholder-shown:scale-100 peer-placeholder-shown:text-white/50"
              >
                Email Address
              </label>
              <div className="absolute right-4 top-4 text-white/20">
            ...
            >
              {isLoading ? (
                ...
                <>
                  <span className="group-hover:translate-x-1 transition-transform">Join Squad</span>
            ...
            <div className="text-center">
              <p className="text-white/50 text-sm">
                Already have an account? <a href="./login" className="font-bold text-[#CBF928] hover:text-white transition-colors">Log In</a>
              </p>
            </div>
          </form>
        </div>
      ...
      <div className={`fixed bottom-8 bg-white/90 backdrop-blur-md px-6 py-4 rounded-full shadow-2xl transform transition-all duration-500 ease-[cubic-bezier(0.68,-0.55,0.27,1.55)] z-50 flex items-center gap-3 ${showToast ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-20 opacity-0 scale-90'}`}>
        ...
        <div>
          <h4 className="font-bold text-gray-900 text-sm">Welcome aboard!</h4>
          <p className="text-gray-500 text-xs">Your account has been created.</p>
        </div>
      </div>
    </div>
  );
...
export default FoodieSignupCard;